...
REMOVED = {
    "auth_verifier",
    "diagnostics",
    "diagnostics_route_test",
    "get_agent_channel_presence",
    "update_agent_channel_presence",
    "get_gitlab_integration",
    "update_gitlab_integration",
    "sweep_support_servicedesk",
}
...
def delete_extracted_definitions() -> int:
    ...
    if missing:
        raise SystemExit(f"Expected extracted system/integration route definitions were not found: {missing}")
    drop: set[int] = set()
...
if __name__ == "__main__":
    size = delete_extracted_definitions()
    tighten_budget(size)
    print(f"legacy_core.py shrunk to {size} bytes; removed {len(REMOVED)} system/integration route definitions")

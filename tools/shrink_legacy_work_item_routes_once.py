...
REMOVED = {
    ...
    "get_work_item",
    "create_work_item_handoff",
    "ack_work_item_handoff",
...
def delete_extracted_definitions() -> int:
    ...
    if missing:
        raise SystemExit(f"Expected extracted work-item route definitions were not found: {missing}")
    drop: set[int] = set()
...
def tighten_budget(size: int) -> None:
    ...
    source, count = re.subn(r"^MAX_LEGACY_CORE_BYTES = \d+$", f"MAX_LEGACY_CORE_BYTES = {size}", source, count=1, flags=re.MULTILINE)
    if count != 1:
...
if __name__ == "__main__":
    ...
    print(f"legacy_core.py shrunk to {new_size} bytes; removed {len(REMOVED)} work-item API route definitions")

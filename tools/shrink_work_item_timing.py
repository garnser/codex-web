...
WORKFLOW = ROOT / ".github" / "workflows" / "shrink-work-item-timing.yml"
SELF = Path(__file__)
TARGETS = {
...
import_anchor = "from codex_web.services.work_item_state import install_work_item_state_machine\n"
new_import = "from codex_web.services.work_item_timing import install_work_item_timing_policy\n"
if new_import not in application:
    if application.count(import_anchor) != 1:
        raise SystemExit("could not find unique work-item service import anchor")
    application = application.replace(import_anchor, import_anchor + new_import, 1)

install_anchor = "turn_execution_service = install_turn_execution_service(app, core)\nautonomy_service = install_autonomy_service(app, core)\n"
replacement = (
...
if "work_item_timing_policy = install_work_item_timing_policy(app, core)" not in application:
    if application.count(install_anchor) != 1:
        raise SystemExit("could not find unique autonomy composition anchor")
...
print(f"legacy_core.py -> {new_size} bytes; extracted {len(TARGETS)} timing helpers")

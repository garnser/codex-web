...
REMOVED = {
    ...
    "get_thread_settings",
    "update_thread_primary",
    "update_thread_primary_channel",
    "archive_thread",
    "unarchive_thread",
    "interrupt_turn",
}
...
def delete_extracted_definitions() -> int:
    ...
    if missing:
        raise SystemExit(f"Expected extracted thread/turn route definitions were not found: {missing}")
...
if __name__ == "__main__":
    ...
    print(f"legacy_core.py shrunk to {new_size} bytes; removed {len(REMOVED)} thread/turn route definitions")

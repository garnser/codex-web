...
async function refreshExtensionAdmin() {
  ...
  try {
    const extensions = await api("/api/extensions");
    let packages = { items: [], errors: [] };
    try {
      packages = await api("/api/extensions/packages");
    } catch (error) {
...
async function mutateExtensionLifecycle(button) {
  ...
  try {
    ...
    await api(`/api/extensions/${encodeURIComponent(installationId)}/${action}`, options);
...
$("extension-admin-list").addEventListener("click", (event) => {

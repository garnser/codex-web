...
class WorkItemTimingPolicy:
    ...
    def sla_threshold_seconds(self, state: WorkItemState) -> float:
        ...
        accepted_by_current_owner = (
            ...
            and h._coerce_owner(state.current_owner) == h._coerce_owner(state.handoff.to_agent)
        )
        ...
        if accepted_by_current_owner:
            ...
            return min(baseline, self.accepted_handoff_owner_idle_seconds())
        if release_stage:
            return self.release_validation_sla_seconds()
        return self.progress_sla_seconds()


def install_work_item_timing_policy(app: Any, host: Any) -> WorkItemTimingPolicy:
    existing = getattr(app.state, "work_item_timing_policy", None)
    if isinstance(existing, WorkItemTimingPolicy) and existing.host is host:
        policy = existing
    else:
        policy = WorkItemTimingPolicy(host)
        app.state.work_item_timing_policy = policy

    host._work_item_handoff_timeout_seconds = policy.handoff_timeout_seconds
    host._work_item_progress_sla_seconds = policy.progress_sla_seconds
    host._release_validation_sla_seconds = policy.release_validation_sla_seconds
    host._accepted_handoff_owner_idle_seconds = policy.accepted_handoff_owner_idle_seconds
    host._work_item_sla_threshold_seconds = policy.sla_threshold_seconds
    return policy

...
class BotDetailService:
    ...
    def record(self, thread_id: str, item_type: str, title: str, text: str) -> None:
        if not text.strip():
            return
        details = self.host._load_bot_details()
        ...
        self.host._save_bot_details(details)

    def latest(self, thread_id: str) -> BotThreadDetail | None:
        items = self.host._load_bot_details().get(thread_id) or []
...
def install_bot_detail_service(app: Any, host: Any) -> BotDetailService:
    existing = getattr(app.state, "bot_detail_service", None)
    if isinstance(existing, BotDetailService) and existing.host is host:
        service = existing
    else:

...
def _devhealth_work_item_stats() -> dict[str, int]:
    return app.state.runtime_diagnostics_service.work_item_stats()
...
def _daemon_health() -> dict[str, Any]:
    return app.state.runtime_health_service.snapshot()

def _static_version() -> str:
    return app.state.static_asset_version_service.version()

def _tail_text_lines(path: Path, limit: int) -> list[str]:
    return app.state.bot_runtime_telemetry.tail_text_lines(path, limit)

def _recent_bot_events(limit: int = 80) -> list[dict[str, Any]]:
    return app.state.bot_runtime_telemetry.recent_events(limit)

def _thread_recent_activity_age_seconds(
    thread_id: str | None,
    *,
    limit: int = 200,
) -> float | None:
    return app.state.bot_runtime_telemetry.thread_recent_activity_age_seconds(
        thread_id,
        limit=limit,
    )
...
def _thread_recent_event_count(
    ...
    *,
    ...
    return app.state.bot_runtime_telemetry.thread_recent_event_count(
        thread_id,
        event_types,
...
def _queued_turn_public(queued: QueuedTurn) -> dict[str, Any]:
    return app.state.runtime_diagnostics_service.queued_turn_public(queued)

def _binding_public(binding: BotBinding) -> dict[str, Any]:
    return app.state.runtime_diagnostics_service.binding_public(binding)
...
def _preview_bot_route(payload: BotRouteTest) -> dict[str, Any]:
    return app.state.bot_routing_service.preview(payload)
...
def _diagnostic_snapshot(project_id: str | None = None) -> dict[str, Any]:
    return app.state.runtime_diagnostics_service.snapshot(project_id)

"""Bootstrap-only execution role seed.

Runtime resolution must use the database-backed Definition Registry. These
values exist only to migrate clean/existing installations to the first ordinary
published definition revision; they are not consulted after bootstrap.
"""
...
SEED_SHARED_EXECUTION_RULES: tuple[str, ...] = (
...
SEED_ROLE_CONTRACTS: dict[str, ExecutionRoleContract] = {
...
SEED_EXECUTIVE_DEFAULT_EXECUTION_ROLE = {
...
def execution_role_catalog_seed_payload() -> dict[str, Any]:
    catalog = ExecutionRoleCatalogDefinition(
        change_classifications=SEED_CHANGE_CLASSIFICATIONS,
        shared_execution_rules=SEED_SHARED_EXECUTION_RULES,
        roles=tuple(SEED_ROLE_CONTRACTS.values()),
        executive_default_execution_role=SEED_EXECUTIVE_DEFAULT_EXECUTION_ROLE,
        owner_to_execution_role=SEED_OWNER_TO_EXECUTION_ROLE,
    )

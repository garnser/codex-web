...
| Work Items launcher/dialog | `static/work_items_ui.js` | redesigned Work Item workspace and Run inspector | migrating | Remove launcher-only compatibility wiring after all supported Work Item entry paths use the redesigned workspace. |
| Agent provider/session admin | `static/agent_provider_admin.js` | Agent/Team workspace surfaces | migrating | Retain provider/runtime administration until replacement mutations and diagnostics have parity. |

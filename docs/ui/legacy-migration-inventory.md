...
| Sidebar Project list | `#projects` rendering in `static/app.js` | canonical Project switcher and project actions in the workspace shell | replacement active | Project switching and creation use the canonical selector/context; project detail and management remain in their owning surfaces. |
| Sidebar Threads list | `#threads` rendering in `static/app.js` | Project-scoped Threads page with contextual sidebar navigation | contextual compatibility | Retain while the Chat page uses the canonical project-scoped thread list; thread-specific deep-link/history behavior remains separately qualified. |

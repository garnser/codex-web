...
| Entitlements / quotas / usage | Y | Y via canonical upsert | Y | N/A | N/A | N/A | capability disable; no hard delete/quota removal API | Y | Configuration / Entitlements. Tenant mode, capability entitlement and quota policy writes require canonical admin authority plus MFA and explicit impact confirmation. Usage remains append-only operational evidence. Capability retirement uses `enabled=false`; quota deletion is deliberately unavailable because the backend exposes no removal contract. Hosted billing authority may remain external. |

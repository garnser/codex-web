...
| Project readiness / bootstrap / task-source authority | Y | API | API | N/A | Y | N/A | API | Y | Project Settings / Work Items. Readiness must surface canonical blockers rather than client-side guesses. |
...
| Execution profiles | Y | API | API | API | Y | API | API | Y | Runtime / Agents / Configuration. Product workflow must preserve exact execution contract and authorization. |

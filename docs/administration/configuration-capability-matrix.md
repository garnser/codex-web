...
| Agent Profiles: identity, role/instruction refs, runtime/model policy, execution profile, sandbox/network requirements, budgets | Y | Y | Y | Y | Y | API | API | Y | Agents. Immutable revision identity is R/O; mutable profile definitions follow canonical revision/lifecycle contracts. |
...
| Teams: membership, delegation policy, budgets, lifecycle | Y | Y | Y | API | Y | API | API | Y | Teams / Squads. Membership and delegated authority remain bounded by canonical policy. |
...
| Recovery / operational policy | Y | Y | Y | explicit policy version + fingerprint | Y key/destination refs | isolated restore verification; production restore is runbook-controlled | retention only; no manual delete API | Y | Operations / Recovery. Policy writes, on-demand encrypted backup and isolated restore verification require admin + MFA or recovery:admin service authority. Backup key material is never exposed; only managed key references are editable. Production restore remains deliberately outside the web mutation API, and backup retirement is canonical retention policy rather than ad-hoc delete. |
